...
for model in "${model_list[@]}"; do
    save_name=lightning_logs/gnn_features
    for ((kfold_id=0; kfold_id<5; kfold_id++)); do
        PROCESSED_DATA="data/MIRAGE/processed/kfold${kfold_id}.pkl"
        ## model training
        python mm gnn_features_ablation mirage $model -config ${CONFIG} -d_kfold $kfold_id -seed $kfold_id -d_processed_data $PROCESSED_DATA -save $save_name
    done
