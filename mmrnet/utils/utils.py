...
def summarize_tensorboard_logs_with_hparams(folder_path):
    """
...
def process_tensorboard_logs(folder_paths, group_keys, save_path=None):
    """
    ...
    """
    summary = summarize_and_format_tensorboard_logs(folder_paths, group_keys)
    summary = add_aggregated_columns(summary)
...
def add_aggregated_columns(summary):
    """
    Adds aggregated columns to the summary DataFrame.

    Args:
        summary (pd.DataFrame): Summary DataFrame.

    Returns:
        pd.DataFrame: Updated DataFrame with aggregated columns.
    """
    if "model_t_graph" not in summary.columns or "model_t_graph_norm_factor" not in summary.columns:
        print("Warning: 'model_t_graph' or 'model_t_graph_norm_factor' columns not found. Skipping aggregation.")
        return summary
    summary.insert(
        summary.columns.get_loc("model_t_graph"),
        "model_t_graph_aggregated",
        summary["model_t_graph"] * summary["model_t_graph_norm_factor"]
    )
    summary["model_t_graph_aggregated"] = summary["model_t_graph_aggregated"].replace(0, "False")
    return summary
...
def filter_and_clean_summary(summary, group_keys):
    ...
    """
    # Adjust group keys: remove if not relevant, add fallback
    group_keys = [key for key in group_keys if key not in ["model_t_graph", "model_t_graph_norm_factor"]]
    if "model_t_graph_aggregated" in summary.columns:
        group_keys += ["model_t_graph_aggregated"]

    # Filtering rows with time stretching only if relevant columns exist
    required_filter_cols = ['dataset_data_augmentation', 'model_t_conv', 'model_t_graph']
    if all(col in summary.columns for col in required_filter_cols):
        try:
            summary = summary[~(
                (summary['dataset_data_augmentation'].astype(str).str.contains("time_stretching")) &
                (~summary['model_t_conv']) &
                (~summary['model_t_graph'])
            )]
        except Exception as e:
            print(f"Warning during filtering by time stretching: {e}")
    ...
    optional_drop_cols = ['model_t_graph', 'model_t_graph_norm_factor']
    drop_cols += [col for col in optional_drop_cols if col in summary.columns]
...
def dataframe_to_pptx(df, pptx_path):
    """
    Exports a DataFrame to a PowerPoint presentation.

    Args:
        df (pd.DataFrame): DataFrame to export.
        pptx_path (str): Path to save the PowerPoint file.
    ...
    # Add more logic to populate the slide with DataFrame content
    prs.save(pptx_path)

def summarize_and_format_tensorboard_logs(folder_paths, group_keys=[]):
    """
    ...
    """
    summaries = [summarize_tensorboard_logs_with_hparams(folder) for folder in folder_paths]
